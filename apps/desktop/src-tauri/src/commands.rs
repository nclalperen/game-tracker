...
use crate::steam::{
  ...
  Price as SteamStorePrice, RecentGame, SteamProfile, WishlistRow,
...
#[tauri::command]
pub fn pack_diagnostics_cmd(app: tauri::AppHandle, payload: DiagnosticsPayload) -> Result<String, String> {
  let cache_dir = app
    .path()
    .app_cache_dir()
    .map_err(|e| format!("Unable to resolve cache directory: {}", e))?;
  fs::create_dir_all(&cache_dir)
    .map_err(|e| format!("Unable to create cache directory {} :: {}", cache_dir.display(), e))?;

  let timestamp = Utc::now().format("%Y%m%d-%H%M%S").to_string();
  let filename = format!("gametracker-diagnostics-{}.zip", timestamp);
  let archive_path = cache_dir.join(&filename);
  let file = fs::File::create(&archive_path)
    .map_err(|e| format!("Unable to create diagnostics archive {} :: {}", archive_path.display(), e))?;

  let mut writer = ZipWriter::new(file);
  let options = FileOptions::default().compression_method(CompressionMethod::Deflated);

  let logs_json = serde_json::to_vec_pretty(&payload.logs).map_err(|e| format!("Serialize logs failed: {}", e))?;
  writer
    .start_file("logs.json", options)
    .map_err(|e| format!("Write logs.json failed: {}", e))?;
  writer
    .write_all(&logs_json)
    .map_err(|e| format!("Write logs.json failed: {}", e))?;

  let digests_json =
    serde_json::to_vec_pretty(&payload.digests).map_err(|e| format!("Serialize digests failed: {}", e))?;
  writer
    .start_file("digests.json", options)
    .map_err(|e| format!("Write digests.json failed: {}", e))?;
  writer
    .write_all(&digests_json)
    .map_err(|e| format!("Write digests.json failed: {}", e))?;

  let settings_json =
    serde_json::to_vec_pretty(&payload.settings).map_err(|e| format!("Serialize settings failed: {}", e))?;
  writer
    .start_file("settings.json", options)
    .map_err(|e| format!("Write settings.json failed: {}", e))?;
  writer
    .write_all(&settings_json)
    .map_err(|e| format!("Write settings.json failed: {}", e))?;

  let mut versions_map: Map<String, Value> = Map::new();
  if let Some(Value::Object(obj)) = payload.versions {
    for (key, value) in obj {
      versions_map.insert(key, value);
    }
  }
  let package = app.package_info();
  versions_map.insert("appName".into(), json!(package.name.clone()));
  versions_map.insert("appVersion".into(), json!(package.version.to_string()));
  versions_map.insert("platform".into(), json!(std::env::consts::OS));
  versions_map.insert("arch".into(), json!(std::env::consts::ARCH));
  match crate::ally::ally_version(&app) {
    Ok(value) => {
      versions_map.insert("allyVersion".into(), json!(value));
    }
    Err(err) => {
      versions_map.insert("allyVersionError".into(), json!(err));
    }
  }
  let versions_json =
    serde_json::to_vec_pretty(&Value::Object(versions_map)).map_err(|e| format!("Serialize versions failed: {}", e))?;
  writer
    .start_file("versions.json", options)
    .map_err(|e| format!("Write versions.json failed: {}", e))?;
  writer
    .write_all(&versions_json)
    .map_err(|e| format!("Write versions.json failed: {}", e))?;

  let mut console_lines = payload.console;
  if console_lines.len() > 50 {
    console_lines = console_lines.split_off(console_lines.len().saturating_sub(50));
  }
  let console_payload = console_lines.join("\n");
  writer
    .start_file("console.log", options)
    .map_err(|e| format!("Write console.log failed: {}", e))?;
  writer
    .write_all(console_payload.as_bytes())
    .map_err(|e| format!("Write console.log failed: {}", e))?;

  writer
    .finish()
    .map_err(|e| format!("Finalize diagnostics archive failed: {}", e))?;
...
pub fn steam_get_wishlist(steamid: String, cc: String, lang: String) -> Result<Vec<WishlistRow>, String> {
  ...
  steam::get_wishlist(trimmed, cc.trim(), lang.trim()).map_err(|e| e.to_string())
...
pub fn sessions_enable_cmd(app: tauri::AppHandle, enable: bool) -> Result<(), String> {

  sessions::sessions_enable(app, enable)
...
pub fn sessions_now_playing_cmd() -> Result<Option<String>, String> {

  Ok(sessions::sessions_now_playing())
