...
import { fetchSteamPriceTRY } from "@/desktop/bridge";
...
export default function LibraryPage() {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Cards */}
      {view === "cards" && (
        //old <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        <div className="grid gap-3 [grid-template-columns:repeat(auto-fill,minmax(var(--card-w,280px),1fr))]">
          {grouped.map((g) => (
            ...
            <tbody>
              {filtered.map((row) => (
                <tr key={row.id}>
                  ...
                  <td className="px-2 py-1">{row.priceTRY ?? "—"}</td>
                  <td className="px-2 py-1">{row.ttbMedianMainH ?? "—"}</td>
                  <td className="px-2 py-1">{pricePerHour(row.priceTRY, row.ttbMedianMainH) ?? "—"}</td>
              ...
              try {
                ...
                if (meta.mainMedianHours != null) {
                  ...
                  await db.library.update(r.id, {
                    ...
                    ttbSource: meta.source === "cache" ? "hltb-cache" : "hltb",
                  } as any);
                } else {
          ...
          <button
            ...
            onClick={async () => {
              try {
                ...
                const p = await fetchSteamPriceTRY(currentAppid, "tr" as any);
                if (p == null) return alert("No price available for this region.");
                setPrice(p);
                await db.library.update(r.id, { priceTRY: p } as any);
              } catch (e: any) {
                alert(e?.message || String(e));
              } finally {
                setBusyPrice(false);
